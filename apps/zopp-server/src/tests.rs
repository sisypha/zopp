...
mod handler_tests {
    ...
    #[tokio::test]
    async fn handler_get_workspace_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gwp1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_get_workspace_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gwp2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
        ...
        let request = create_signed_request(
            ...
            zopp_proto::GetWorkspacePermissionRequest {
                workspace_name: "ws".to_string(),
                principal_id: "invalid-uuid".to_string(),
            },
        );
    ...
    #[tokio::test]
    async fn handler_get_workspace_permission_perm_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gwp3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;

        // Create another principal without permission
        let other_signing_key = ed25519_dalek::SigningKey::generate(&mut rand_core::OsRng);
        let other_principal_id = server
            .store
            .create_principal(&zopp_storage::CreatePrincipalParams {
                user_id: None,
                name: "other".to_string(),
                public_key: other_signing_key.verifying_key().to_bytes().to_vec(),
                x25519_public_key: None,
            })
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn handler_list_workspace_permissions_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lwp1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_remove_workspace_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rwp1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_remove_workspace_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rwp2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    // Project level principal permission error path tests

    #[tokio::test]
    async fn handler_set_project_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-spp1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_set_project_permission_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-spp2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_set_project_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-spp3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_set_project_permission_invalid_role() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-spp4@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_project_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gpp1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_get_project_permission_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gpp2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_project_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gpp3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_project_permission_perm_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gpp4@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;

        // Create another principal without project permission
        let other_signing_key = ed25519_dalek::SigningKey::generate(&mut rand_core::OsRng);
        let other_principal_id = server
            .store
            .create_principal(&zopp_storage::CreatePrincipalParams {
    ...
    #[tokio::test]
    async fn handler_list_project_permissions_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lpp1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_list_project_permissions_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lpp2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_remove_project_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rpp1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_remove_project_permission_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rpp2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_remove_project_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rpp3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    // Environment level principal permission error path tests

    #[tokio::test]
    async fn handler_set_environment_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-sep1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_set_environment_permission_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-sep2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_set_environment_permission_env_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-sep3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_set_environment_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-sep4@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_set_environment_permission_invalid_role() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-sep5@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_environment_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gep1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_get_environment_permission_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gep2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_environment_permission_env_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gep3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_environment_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gep4@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_environment_permission_perm_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gep5@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;

        // Create another principal without env permission
        let other_signing_key = ed25519_dalek::SigningKey::generate(&mut rand_core::OsRng);
        let other_principal_id = server
            .store
            .create_principal(&zopp_storage::CreatePrincipalParams {
    ...
    #[tokio::test]
    async fn handler_list_environment_permissions_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lep1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_list_environment_permissions_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lep2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_list_environment_permissions_env_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lep3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_remove_environment_permission_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rep1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_remove_environment_permission_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rep2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_remove_environment_permission_env_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rep3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_remove_environment_permission_invalid_principal_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rep4@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    // ================== Environment Handler Service Account Tests ==================

    #[tokio::test]
    async fn handler_create_environment_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-cenv-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "env-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_list_environments_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-lenv-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "env-list-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
        ...
        let result = server.list_environments(request).await;
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().code(), tonic::Code::Unauthenticated);
    }

    #[tokio::test]
    async fn handler_delete_environment_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-denv-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;
        ...
        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "env-delete-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_environment_service_principal_success() {
        let server = create_test_server().await;
        let (user_id, principal_id, _signing_key) =
            create_test_user(&server, "owner-genv-sp1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
        ...
        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "env-get-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;

        // Grant read permission to service principal at workspace level
        server
            .store
            .set_workspace_permission(&ws_id, &service_principal_id, Role::Read)
            .await
        ...
        let result = server.get_environment(request).await;
        assert!(result.is_ok());
        let env = result.unwrap().into_inner();
        assert_eq!(env.name, "dev");
    }

    #[tokio::test]
    async fn handler_get_environment_service_principal_workspace_not_found() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-genv-sp2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create a service principal but don't add to workspace
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "env-get-service-bot2").await;
        // Note: not adding principal to workspace - they won't have access

        // Still grant permission (though won't matter without workspace access)
        server
            .store
            .set_workspace_permission(&ws_id, &service_principal_id, Role::Read)
            .await
    ...
    #[tokio::test]
    async fn handler_create_environment_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-cenv-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_create_environment_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-cenv-nf2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_list_environments_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lenv-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_list_environments_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lenv-nf2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_delete_environment_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-denv-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_delete_environment_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-denv-nf2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_environment_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-genv-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_get_environment_project_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-genv-nf2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    #[tokio::test]
    async fn handler_create_environment_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-cenv-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;

        // Create another user with only READ permission (not ADMIN which is required for create)
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-cenv-pd1@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
    ...
    #[tokio::test]
    async fn handler_delete_environment_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-denv-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let proj_id = create_test_project(&server, &ws_id, "proj").await;
        let _env_id = create_test_environment(&server, &proj_id, "dev").await;

        // Create another user with only READ permission (not ADMIN which is required for delete)
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-denv-pd1@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
    ...
    #[tokio::test]
    async fn handler_list_environments_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-lenv-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;

        // Create another user without any permissions
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-lenv-pd1@example.com", "laptop").await;
        // Add them to workspace membership (so they can look it up) but don't grant any permission
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
    ...
    #[tokio::test]
    async fn handler_create_invite_invalid_workspace_id() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-cinv1@example.com", "laptop").await;
        let _ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &_ws_id, &principal_id).await;
        ...
        let request = create_signed_request(
            ...
            "/zopp.ZoppService/CreateInvite",
            zopp_proto::CreateInviteRequest {
                workspace_ids: vec!["not-a-uuid".to_string()],
    ...
    #[tokio::test]
    async fn handler_create_invite_invalid_expires_at() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-cinv2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
        ...
        let request = create_signed_request(
            ...
            zopp_proto::CreateInviteRequest {
                ...
                kek_encrypted: vec![],
                kek_nonce: vec![],
                expires_at: i64::MIN, // Invalid timestamp
            },
        );
    ...
    #[tokio::test]
    async fn handler_create_invite_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-cinv3@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "invite-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_list_invites_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-linv1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "invite-list-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
        ...
        let result = server.list_invites(request).await;
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().code(), tonic::Code::Unauthenticated);
    }

    #[tokio::test]
    async fn handler_revoke_invite_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-rinv1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "invite-revoke-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
        ...
        let request = create_signed_request(
            ...
            &service_signing_key,
            "/zopp.ZoppService/RevokeInvite",
            zopp_proto::RevokeInviteRequest {
                token: "some-token".to_string(),
            },
        );

        let result = server.revoke_invite(request).await;
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().code(), tonic::Code::Unauthenticated);
    }

    #[tokio::test]
    async fn handler_revoke_invite_not_found() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-rinv2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
    ...
    async fn handler_revoke_invite_no_admin_permission() {
        ...
        let invite = server
            ...
            .create_invite(&zopp_storage::CreateInviteParams {
                workspace_ids: vec![ws_id.clone()],
                token: "test-revoke-token".to_string(),
                kek_encrypted: None,
                kek_nonce: None,
        ...
        // Create another user without admin permission
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-rinv3@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
            .unwrap();
        add_principal_to_workspace(&server, &ws_id, &other_principal_id).await;
        // Grant only READ permission (not ADMIN)
        server
            .store
            .set_workspace_permission(&ws_id, &other_principal_id, Role::Read)
            .await
    ...
    #[tokio::test]
    async fn handler_create_invite_success() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-cinv4@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
        ...
        let request = create_signed_request(
            ...
            zopp_proto::CreateInviteRequest {
                workspace_ids: vec![ws_id.0.to_string()],
                token: "test-token-success".to_string(),
        ...
        let result = server.create_invite(request).await;
        assert!(result.is_ok());
        let invite = result.unwrap().into_inner();
        assert!(!invite.id.is_empty());
        assert_eq!(invite.workspace_ids.len(), 1);
    }

    #[tokio::test]
    async fn handler_list_invites_success() {
        let server = create_test_server().await;
        let (user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-linv2@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        add_principal_to_workspace(&server, &ws_id, &principal_id).await;
        ...
        server
            ...
            .create_invite(&zopp_storage::CreateInviteParams {
                workspace_ids: vec![ws_id.clone()],
                token: "test-list-token".to_string(),
                kek_encrypted: None,
                kek_nonce: None,
                expires_at: chrono::Utc::now() + chrono::Duration::hours(1),
                created_by_user_id: Some(user_id.clone()),
        ...
        let result = server.list_invites(request).await;
        assert!(result.is_ok());
        let invites = result.unwrap().into_inner();
        assert!(!invites.invites.is_empty());
    }

    // ================== Project Handler Service Account Tests ==================

    #[tokio::test]
    async fn handler_create_project_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-cproj-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "project-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_list_projects_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-lproj-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "project-list-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
        ...
        let result = server.list_projects(request).await;
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().code(), tonic::Code::Unauthenticated);
    }

    #[tokio::test]
    async fn handler_get_project_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-gproj-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "project-get-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_delete_project_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-dproj-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "project-delete-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_list_projects_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lproj-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_get_project_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-gproj-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_delete_project_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-dproj-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_list_projects_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-lproj-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create another user without any permission
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-lproj-pd1@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
    ...
    #[tokio::test]
    async fn handler_delete_project_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-dproj-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _proj_id = create_test_project(&server, &ws_id, "proj").await;

        // Create another user with READ permission (not ADMIN)
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-dproj-pd1@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
    ...
    // ================== Group Handler Additional Tests ==================

    #[tokio::test]
    async fn handler_get_group_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-ggrp-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _group_id = create_test_group(&server, &ws_id, "my-group").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "group-get-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_list_groups_service_account_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-lgrp-s1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create a service principal
        let (service_principal_id, service_signing_key) =
            create_service_principal(&server, "group-list-service-bot").await;
        add_principal_to_workspace(&server, &ws_id, &service_principal_id).await;
    ...
    #[tokio::test]
    async fn handler_get_group_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-ggrp-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_list_groups_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-lgrp-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_create_group_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-cgrp-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;

        // Create another user with READ permission (not ADMIN)
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-cgrp-pd1@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
    ...
    #[tokio::test]
    async fn handler_delete_group_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-dgrp-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _group_id = create_test_group(&server, &ws_id, "my-group").await;

        // Create another user with READ permission (not ADMIN)
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-dgrp-pd1@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
    ...
    #[tokio::test]
    async fn handler_update_group_permission_denied() {
        let server = create_test_server().await;
        let (user_id, _principal_id, _signing_key) =
            create_test_user(&server, "owner-ugrp-pd1@example.com", "laptop").await;
        let ws_id = create_test_workspace(&server, &user_id, "ws").await;
        let _group_id = create_test_group(&server, &ws_id, "my-group").await;

        // Create another user with READ permission (not ADMIN)
        let (other_user_id, other_principal_id, other_signing_key) =
            create_test_user(&server, "other-ugrp-pd1@example.com", "laptop").await;
        server
            .store
            .add_user_to_workspace(&ws_id, &other_user_id)
            .await
        ...
        let request = create_signed_request(
            ...
            zopp_proto::UpdateGroupRequest {
                workspace_name: "ws".to_string(),
                group_name: "my-group".to_string(),
                new_name: "renamed-group".to_string(),
                new_description: String::new(),
            },
        );
    ...
    #[tokio::test]
    async fn handler_update_group_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-ugrp-nf1@example.com", "laptop").await;
    ...
    #[tokio::test]
    async fn handler_delete_group_workspace_not_found() {
        let server = create_test_server().await;
        let (_user_id, principal_id, signing_key) =
            create_test_user(&server, "owner-dgrp-nf1@example.com", "laptop").await;
    ...
    // ==================== Issue #45: RBAC enforcement tests ====================
